...
\begin{abstract}
	...
	Die geringe Rechenleistung von mobilen Endgeräten und eingebetteten Systemen stellt jedoch den Einsatz dieser Technologie auf diesen Geräten vor eine Herausforderung. Verschiedene Ansätze, um die Rechenintensität eines Einlern-/ und Wiedererkennungsalgorithmus gering zu halten, sind bereits verfügbar und einsetzbar. \\
...
\end{itemize}
Im Folgenden werden die Anforderungen an das zu entwickelnde Softwaresystem thematisch gruppiert und in funktionale und nicht-funktionale Anforderungen unterteilt.
...
Eine auf einem aktuellen Android-Betriebssystem lauffähige Applikation soll entwickelt und auf einem Smartphone in Betrieb genommen werden. Es wird gefordert, dass der Nutzer des Software-Systems die im Smartphone integrierte Kamera nutzt und diese als Eingabe für das Einlernen und die Wiedererkennung eines Objektes nutzt.  -- \textit{Gewichtung:} Must-Have-Anforderung.
...
Um die Übersichtlichkeit und der darausfolgenden besseren Wart- und Testbarkeit der Applikation zu gewährleisten, werden Dialogboxen ausgelagert und mithilfe einer Schnittstelle und einer \textit{Factory-Method} erzeugt. Dialogboxen werden zur Interaktion mit dem Nutzer häufig verwendet. Das Hinzufügen oder Ändern von Daten, sowie das Anzeigen von längeren Hinweisen wird über Dialogboxen an den Nutzer kommuniziert. Bei kritischen nicht-umkehrbaren Aktionen, wie beispielsweise dem Löschen von Objekten oder dem Zurücksetzen der Applikation, wird der Nutzer ebenfalls durch eine Dialogbox auf die möglichen Auswirkungen der Aktion hingewiesen.\\
...
Nach Konfiguration der soeben beschriebenen Parameter, wird die Kamera an den Lebenszyklus des Fragments gebunden und demnach bei Öffnen des Fragments gestartet. Beim Schließen beziehungsweise Wechseln des Fragments wird die Kamera geschlossen.\\
...
Methoden, in welchen die neuen Trainingsdaten an das Modell weitergegeben werden, werden durch eine Replay-Methode erweitert. Daraufhin werden die Trainingsdaten zusammen mit bereits vorhandenen Trainingsdaten trainiert und das Phänomen des Katastrophalen Vergessens kann gelöst werden.
...
Die Shared Preferences können durch einfache Befehle, wie in Quelltext 5.2 dargestellt, gesetzt und abgefragt werden. Der Schlüssel des gespeicherten Key-Value-Paares wird beim Setzen der Shared Preference initialisiert, falls er noch nicht vorhanden ist und aktualisiert, wenn ein Key mit dem angegebenen Namen bereits existiert.\\
...
\section{Unit-Tests}
Für die Implementierung der zu schreibenden Tests, werden das Test-Framework \textit{JUnit} und \textit{AndroidX}-Testbibliotheken standardmäßig von Android Studio bei Erstellung eines Projektes eingebunden und demnach auch für das Testen der zu entwickelnden Applikation genutzt. Für die Assertions der Tests wird für eine bessere Lesbarkeit die von Google zur Verfügung gestellte Bibliothek \textit{Truth} verwendet.\\
Für das Erstellen von instrumentierten Unit-Tests, welche auf einem Android-Emulator ausgeführt werden, werden AndroidX Tests in einer Ordnerstruktur angelegt, welche der Struktur des Hauptprojektes entspricht. Diese instrumentierten Unit-Tests sind notwendig für das Testen von Methoden, welche Android-spezifische Abhängigkeiten enthalten, wie beispielsweise \textit{Activities} oder \textit{Contexts} und nicht lokal auf dem Entwicklungsrechner ausführbar sind. Alle anderen Tests werden mit \textit{JUnit} geschrieben und sind demnach schneller ausführbar, da sie in der lokalen Java Virtual Machine (JVM) ausgeführt werden können. Die Verwendung eines Emulators ist somit nicht notwendig.\\
...
Hierfür wurden Testszenarien für die einzelnen Methoden erstellt. Mithilfe von Assertions konnte die Ausgabe beziehungsweise die Folgen der Ausführung der Methode einzeln getestet werden. Durch das Implementieren der \texttt{setUp}- und \texttt{tearDown}-Methode, welche vor beziehungsweise nach Durchlauf des Unit-Tests  ausgeführt werden, konnte für die Unit-Tests Boilerplate-Code gespart werden, welcher für das Erstellen und Schließen des Datenbanktreibers notwendig ist. Dieses Prinzip gewährleistet zudem die Unabhängigkeit der einzelnen Tests, da diese sich nicht beeinflussen dürfen. Demnach wäre das Verwenden einer gemeinsamen Datenbankhelfer-Instanz nicht möglich.\\
Ein Beispiel Unit-Test, welcher die Methode, die für das Aktualisieren des Vorschaubildes eines Objektes zuständig ist, testet diese Methode auf gewisse Grenzfälle und wird in Quelltext 6.1 dargestellt.\\

\begin{minipage}{\linewidth}
	\begin{lstlisting}[language=Java, breaklines=true, caption={Ausschnitt aus Unit-Test für Datenbankhelfer-Methoden}]
	// ...
	
    @Test
	public void updateImageBlob_objectNameIsInvalidOrNull_doesNothing() {
		// Save string representation of DB before method
		String before = dbHelper.cursorToString(dbHelper.getAllObjects());
		// Run with invalid name
		dbHelper.updateImageBlob("invalid object", Bitmap.createBitmap(10,10,Bitmap.Config.ARGB_8888));
		String afterInvalid = dbHelper.cursorToString(dbHelper.getAllObjects());
		// Run with null as name
	...
	\end{lstlisting}
\end{minipage}
\label{lst:unit tests}
Um eine vollständig getestete Applikation gewährleisten zu können wären Integrationstests, die das Zusammenarbeiten der einzelnen Komponenten Testen, sowie UI-Tests, die die graphische Nutzeroberfläche automatisiert testen notwendig gewesen.
...
Im Folgenden werden die erreichten Ziele und Ergebnisse dieser Arbeit zusammengefasst.\\ Für die Weiterentwicklung der erstellten Lösung werden weitere Ideen aufgezeigt, um den gegebenen Anwendungsfall zu erweitern und um die erreichten Ergebnisse auf andere Problemstellungen übertragen zu können.
\section{Erreichte Ergebnisse}
\label{sec:ergebnisse}
Das zu Beginn der Arbeit gesetzte Ziel der Implementierung einer Augmented-Reality-Applikation für das Wiedererkennen bereits eingelernter Objekte, wurde mithilfe verschiedener Deep Learning Technologien und Werkzeugen, wie beispielsweise Continual Learning und Tensorflow Lite für den Einsatz auf einem mobilen Android-Gerät unter Nutzung der integrierten Kamera, erreicht. Die in der Anforderungsanalyse aufgearbeiteten funktionalen und nicht-funktionalen Anforderungen konnten ebenfalls größtenteils erfüllt werden.\\
Die Wiedererkennungsqualität in sehr guten Lichtverhältnissen ist aufgrund eines höheren Kontrast der Eingabedaten besser, jedoch ist es auch bei relativ schlechten Lichtverhältnissen möglich einige eingespeicherte Objekte voneinander zu unterscheiden.\\
Die Informationen, welche zum zu erkennenden Objekt gespeichert wurden, werden durch ein Bildschirm-Overlay angezeigt und ermöglichen es in Echtzeit die Objekte korrekt zu klassifizieren. Somit ist ein Augmented-Reality-Erlebnis gewährleistet.\\
Aufgrund des Phänomen des Katastrophalen Vergessens ist der Puffer des Latent-Replay-Algorithmus beim Schließen und Wechseln der einzelnen Fragments zu aktualisieren, um bereits eingelernte Objekte nicht mit neuen Objekten zu überschreiben. Dies benötigt jedoch bei einer hohen Anzahl von eingespeicherten Objekten eine gewisse Zeit. Dem Nutzer wird dies durch einen Ladekreis mitgeteilt.\\
Das Einlernen der Objekte ist jedoch in einer annehmbaren Zeit möglich. Bei bis zu vier Objekten sind unter Verwendung der Standard-Trainingskonfiguration meistens keine 15 Sekunden für das Einlernen notwendig, um akzeptable Erkennungsergebnisse zu erhalten. Bei zehn Objekten konnte eine maximale Dauer von 70 Sekunden subjektiv festgestellt werden.\\
Bei einer Anzahl von etwa 150 Trainingsdaten pro Objekt, ist eine relativ hohe Erkennungsrate zu erwarten, welche jedoch aufgrund des Fehlens eines für den Anwendungsfall spezifizierten Testframeworks nicht anhand einer Genauigkeitsmetrik gemessen, sondern lediglich subjektiv bewertet wurde.\\
Das Speichern von objektbezogenen Daten konnte mittels SQLite in einer strukturierten relationalen Datenbank persistent abgespeichert und wieder aufgerufen werden.
Durch das Einführen der Objektordnerfunktionalität, ist es demnach möglich beliebig viele Objekte einzulernen.\\
Es ist möglich bis zu zehn Objekte in einen Objektordner einzuspeichern.\\
Eine möglichst intuitive graphische Oberfläche wurde entworfen und anhand eines Betatests evaluiert.\\
Auf die Wart- und Erweiterbarkeit wurde im gesamten Entwurfs- und Implementierungsprozess geachtet. Es wurde versucht die SOLID-Prinzipien einzuhalten, um diese Anforderung erfüllen zu können.\\
Der Quellcode wurde ausführlich dokumentiert und komplexe Algorithmen und Abläufe wurden mithilfe von Kommentaren erklärt. Bei Bedarf ist eine Dokumentation mithilfe von \textit{Doxygen} beziehungsweise von \textit{Javadoc} in wenigen Schritten generierbar.
...
Da auf die Erweiterbarkeit, durch das Entwerfen einer modularen App, bestehend aus  lose-gekoppelten Komponenten, geachtet wurde, ist es möglich einige Erweiterungen hinzuzügen, welche im Folgenden erklärt werden.\\
Es ist möglich, die Applikation um eine Social-Media-Funktionalität zu erweitern und das Teilen von Objektordnern oder einzelnen Objekte zwischen verschiedenen Nutzern zu ermöglichen.\\
Falls die Verwendung eines externen Servers und dem dafür benötigten Zugang zum Internet gewünscht ist, ist es außerdem möglich das gesamte Training auf einen leistungstarken Rechner zu verlagern, um das verwendete mobile Endgerät nicht mit den rechenintensiven Prozessen zu belasten. Dies könnte den Energieverbrauch der Applikation massiv senken, falls dies eine Anforderung für die Weiterentwicklung der App sei.\\
Falls ein nicht vollständig generisches Klassifizierungsmodell gefordert wird, ist es möglich das Modell auf einen spezifischeren Datensatz vorzutrainieren. Dies könnte zwar die Wiedererkennungsgenauigkeit bestimmter Objekte, die keine Ähnlichkeit zum vortrainierten Datensatz aufweisen verschlechtern, aber dafür die Genauigkeit bei ähnlichen Objekten stark verbessern.

\subsection{Übertragbarkeit der Ergebnisse}
\label{sub:uebertragbarkeit}
Die entwickelte Applikation kann mithilfe der angewendeten Technologien, an eine Vielzahl an Anwendungsgebieten angepasst werden.\\
Das Prinzip des Continual Learnings kann zudem nicht nur für Klassifizierungsalgorithmen angewandt werden, sondern auch für andere Teilgebiete des maschinellen Sehens, wie beispielsweise der Objektdetektion oder der semantischen Segmentierung. Hierfür müsste das zugrundeliegende Netz mit einem anderen Deep-Learning-Modell ausgetauscht werden und der Einlernprozess könnte durch das Markieren der Objekte durch das Einzeichnen auf dem Touchscreen erweitert werden. Es ist jedoch zu beachten, dass die eben genannten Anwendungsfälle eine weitaus höhere Rechenleistung benötigen für den Inferenzvorgang.\\
Die Grundfunktionalität der Applikation ist in einer Vielzahl von unterschiedlichen Branchen und Industrien einsetzbar. Es ist beispielsweise möglich für die Realisierung eines \glqq Schnitzeljagd\grqq-Spiels eine Augmented Reality App mithilfe von Continual Learning zu implementieren.\\
Im Bereich des E-Learnings könnte der Unterricht an Schulen durch die Visualisierung von bestimmten Themen unterstützt werden. Lehrer könnten selbsterstellte Klassifizierungsmodelle erstellen, um Schülern eine weitere Visualisierungsmöglichkeit für den Unterricht bieten zu können.
...
\begin{table}[htbp]
	...
	\begin{tabular}{|l|p{80mm}|}
		...
		\textbf{Beschreibung} & Beliebig viele neue Modelle sollen hinzugefügt werden können \\ \hline
		\textbf{Akteur} & Nutzer \\ \hline
		\textbf{Auslöser} & Der Nutzer betätigt den \glqq+\grqq-Button in der Modellübersicht oder startet die Applikation zum ersten Mal  \\ \hline
		\textbf{Vorbedingung} & Der Nutzer befindet sich in der Modellübersicht oder hat bisher noch kein Modell hinzugefügt  \\ \hline	
...
\begin{table}[htbp]
	...
	\begin{tabular}{|l|p{80mm}|}
		...
		\textbf{Standardablauf} & 1. Nutzer betätigt den Button, welcher sich rechts neben dem Modell befindet, um dessen Objekte betrachten zu können\\
...
\begin{table}[htbp]
	...
	\begin{tabular}{|l|p{80mm}|}
		...
		\textbf{Nachbedingung im Sonderfall} & Das Training wurde nicht gestartet\\ \hline
...
\begin{table}[htbp]
	...
	\begin{tabular}{|l|p{80mm}|}
		...
		\textbf{Vorbedingung} & Der Nutzer hat bereits Objekte eingespeichert \\ \hline	
...
\begin{table}[htbp]
	...
	\begin{tabular}{|l|p{80mm}|}
		...
		\textbf{Beschreibung} & Die Größe der Fokusbox im Kamerahauptbildschirm soll konfigurierbar sein, um nahe beziehungsweise Objekte in der Ferne zu erfassen, ohne zu viel Hintergrund einzufangen \\ \hline
		\textbf{Akteur} & Nutzer \\ \hline
		\textbf{Auslöser} & Der Nutzer betätigt den jeweiligen Schieberegler in den App-Einstellungen \\ \hline
		\textbf{Vorbedingung} & Der Nutzer befindet sich in den App-Einstellungen \\ \hline	
